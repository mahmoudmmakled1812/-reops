...
public class ProductController : Controller
{
    ...
    private IFileUpload fileUpload = new FileUpload();
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct = default)
    {
        ...
        {
            var mainImgPath = fileUpload.GenerateFullPath(FileType.Img, "products", product.MainImg);
            if (mainImgPath is not null)
                fileUpload.DeleteFileLocally(mainImgPath);
        }

        var productSubImgs = _productSubImgRepository.Get(e => e.ProductId == product.Id);
        foreach (var subImg in productSubImgs)
        {
            var subImgPath = fileUpload.GenerateFullPath(FileType.Img, "products\\sub-imgs", subImg.SubImg);
            if (subImgPath is not null)
                fileUpload.DeleteFileLocally(subImgPath);
        }
